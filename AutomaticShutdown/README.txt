...
## DEPLOY INSTRUCTIONS
	...
	- If you want to automatically shutdown computers running Windows in your Network, 
	  make sure that all computers have .NET Framework 4.6.1 installed.

	- In each computer, deploy AS.exe, KeepAliveAS.exe and AS.config
	
	- In each computer edit AS.config for changing the GLOBAL_CONFIGURATOR. 
	  It should point to a file name shared in your network, which will be visible for all computers. I.e.:
	...
	- Edit the global configuration file \\A-REMOTE-HOST\SHARED-FOLDER\as.global.cfg and add a 
	  shutdown policy configuration for all computers:
...
## DEBUG INSTRUCTIONS
    ...
    - Make sure you have a Postbuild command in AS project such as 
	  ...
	  This will copy AS outputs (exe and config) to KeepAliveAS folder, as keep alive tries to find in current path AS.exe 
	  in order to keep it runnning
